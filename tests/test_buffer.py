...
class TestWriteBuffer:
    def test_append_and_flush(self):
        ...
        buf.append({"content_hash": "a", "source": "r", "source_name": "t",
                     "timestamp": "2026-07-20T00:00:00Z", "chain_hash": "c",
                     "previous_hash": "p"})
    ...
    def test_auto_flush_on_buffer_full(self):
        ...
        for i in range(3):
            buf.append({"content_hash": f"h{i}", "source": "r", "source_name": "t",
                         "timestamp": "2026-07-20T00:00:00Z", "chain_hash": f"c{i}",
                         "previous_hash": f"p{i}"})
...
class TestContextTrailBuffered:
    def test_buffered_log_and_flush(self):
        trail = ContextTrail(backend="memory", buffered=True, buffer_size=100, flush_interval=60)
    ...
    def test_buffered_auto_flush_on_full(self):
        trail = ContextTrail(backend="memory", buffered=True, buffer_size=5, flush_interval=60)
        for i in range(5):
            trail.log(f"data{i}", source="retriever")
...
class TestBenchmarks:
    def test_log_10k_entries_under_10s(self):
        trail = ContextTrail(backend="memory", buffered=True, buffer_size=500, flush_interval=60)
